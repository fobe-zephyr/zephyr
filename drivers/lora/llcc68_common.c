...
LOG_MODULE_REGISTER(llcc68_common, CONFIG_LORA_LOG_LEVEL);

struct llcc68_rx_params {
    uint8_t *buf;
    uint8_t *size;
    int16_t *rssi;
    int8_t *snr;
...
static struct llcc68_common_data {
    ...
    struct llcc68_rx_params rx_params;
} dev_common_data;
 ...
 *
 * @param data common llcc68 data struct
 *
 * @retval true if modem was acquired
 * @retval false otherwise
 */
static inline bool modem_acquire(struct llcc68_common_data *data) {
 ...
 * @retval false otherwise
 */
static bool modem_release(struct llcc68_common_data *data) {
...
static void llcc68_ev_rx_done(uint8_t *payload, uint16_t size, int16_t rssi,
                              int8_t snr) {
    struct k_poll_signal *sig = dev_common_data.operation_done;

    /* Receiving in asynchronous mode */
    if (dev_common_data.async_rx_cb) {
        /* Start receiving again */
        Radio.Rx(0);
        /* Run the callback */
        dev_common_data.async_rx_cb(dev_common_data.dev, payload, size, rssi, snr);
     ...
     */
    if (!atomic_cas(&dev_common_data.modem_usage, STATE_BUSY, STATE_CLEANUP)) {
        return;
    }
    ...
    /* Store actual size */
    if (size < *dev_common_data.rx_params.size) {
        *dev_common_data.rx_params.size = size;
    }
    /* Copy received data to output buffer */
    memcpy(dev_common_data.rx_params.buf, payload,
           *dev_common_data.rx_params.size);
    /* Output RSSI and SNR */
    if (dev_common_data.rx_params.rssi) {
        *dev_common_data.rx_params.rssi = rssi;
    }
    if (dev_common_data.rx_params.snr) {
        *dev_common_data.rx_params.snr = snr;
    }
    /* Put radio back into sleep mode */
    Radio.Sleep();
    /* Completely release modem */
    dev_common_data.operation_done = NULL;
    atomic_clear(&dev_common_data.modem_usage);
    /* Notify caller RX is complete */
    k_poll_signal_raise(sig, 0);
}

static void llcc68_ev_tx_done(void) {
    struct k_poll_signal *sig = dev_common_data.operation_done;

    if (modem_release(&dev_common_data)) {
...
static void llcc68_ev_tx_timed_out(void) {
    /* Just release the modem */
    modem_release(&dev_common_data);
}
...
int llcc68_lora_send(const struct device *dev, uint8_t *data,
    ...
    if (!dev_common_data.modem_config.frequency) {
        return -EINVAL;
    }

    ret = llcc68_lora_send_async(dev, data, data_len, &done);
    if (ret < 0) {
        return ret;
    }
    ...
    air_time = Radio.TimeOnAir(MODEM_LORA,
                               dev_common_data.modem_config.bandwidth,
                               dev_common_data.modem_config.datarate,
                               dev_common_data.modem_config.coding_rate,
                               dev_common_data.modem_config.preamble_len,
                               0, data_len, true);
...
int llcc68_lora_send_async(const struct device *dev, uint8_t *data,
    ...
    /* Ensure available, freed by llcc68_ev_tx_done */
    if (!modem_acquire(&dev_common_data)) {
        return -EBUSY;
    }

    /* Store signal */
    dev_common_data.operation_done = async;
...
int llcc68_lora_recv(const struct device *dev, uint8_t *data, uint8_t size,
    ...
    /* Ensure available, decremented by llcc68_ev_rx_done or on timeout */
    if (!modem_acquire(&dev_common_data)) {
        return -EBUSY;
    }

    dev_common_data.async_rx_cb = NULL;
    /* Store operation signal */
    dev_common_data.operation_done = &done;
    /* Set data output location */
    dev_common_data.rx_params.buf = data;
    dev_common_data.rx_params.size = &size;
    dev_common_data.rx_params.rssi = rssi;
    dev_common_data.rx_params.snr = snr;
...
int llcc68_lora_recv_async(const struct device *dev, lora_recv_cb cb) {
    ...
    /* Store parameters */
    dev_common_data.async_rx_cb = cb;
...
int llcc68_lora_config(const struct device *dev,
    ...
    memcpy(&dev_common_data.modem_config, config, sizeof(dev_common_data.modem_config));
...
int llcc68_lora_test_cw(const struct device *dev, uint32_t frequency,
                        int8_t tx_power,
                        uint16_t duration) {
    /* Ensure available, freed in llcc68_ev_tx_done */
    if (!modem_acquire(&dev_common_data)) {
        return -EBUSY;
    }
...
int llcc68_lora_is_channel_free(const struct device *dev, int16_t rssi_thresh, uint64_t max_carrier_sense_time) {
    /* Ensure available, freed in llcc68_ev_tx_done */
    if (!modem_acquire(&dev_common_data)) {
        return -EBUSY;
    }
    ...
    Radio.SetChannel(dev_common_data.modem_config.frequency);

    // Set Rx bandwidth. Other parameters are not used.
    Radio.SetRxConfig(MODEM_FSK, dev_common_data.modem_config.bandwidth, 600, 0, dev_common_data.modem_config.bandwidth,
                      3, 0, false,
                      0, false, 0, 0, false, true);
    Radio.Rx(0);
    k_sleep(K_MSEC(1));
    carrier_sense_time = k_uptime_get();
    while ((k_uptime_get()-carrier_sense_time) < max_carrier_sense_time) {
...
int llcc68_init(const struct device *dev) {
    atomic_set(&dev_common_data.modem_usage, 0);

    dev_common_data.dev = dev;
    dev_common_data.events.TxDone = llcc68_ev_tx_done;
    dev_common_data.events.RxDone = llcc68_ev_rx_done;
    /* TX timeout event raises at the end of the test CW transmission */
    dev_common_data.events.TxTimeout = llcc68_ev_tx_timed_out;
    Radio.Init(&dev_common_data.events);
