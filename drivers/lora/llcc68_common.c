		     ...
		     uint32_t data_len)
{
	...
	if (!dev_common_data.tx_cfg.frequency) {
		return -EINVAL;
	}
	...
	air_time = Radio.TimeOnAir(MODEM_LORA,
				   dev_common_data.tx_cfg.bandwidth,
				   dev_common_data.tx_cfg.datarate,
				   dev_common_data.tx_cfg.coding_rate,
				   dev_common_data.tx_cfg.preamble_len,
				   0, data_len, true);
		       ...
		       struct lora_modem_config *config)
{
	...
	if (config->tx) {
		...
		memcpy(&dev_common_data.tx_cfg, config, sizeof(dev_common_data.tx_cfg));
