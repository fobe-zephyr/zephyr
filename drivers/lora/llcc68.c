...
#include "llcc68_common.h"

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(llcc68, CONFIG_LORA_LOG_LEVEL);

BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(semtech_sx1261) +
	     DT_NUM_INST_STATUS_OKAY(semtech_sx1262) +
         DT_NUM_INST_STATUS_OKAY(semtech_llcc68) +
...
#define LLCC68_CALIBRATION_ALL 0x7f

static const struct llcc68_config dev_config = {
...
static struct llcc68_data dev_data;
...
static const char *llcc68_mode_name(RadioOperatingModes_t m)
{
...
static int llcc68_spi_transceive(uint8_t *req_tx, uint8_t *req_rx,
...
void SX126xReadRegisters(uint16_t address, uint8_t *buffer, uint16_t size)
{
	...
	llcc68_spi_transceive(req, NULL, sizeof(req), NULL, buffer, size);
...
void SX126xWriteRegisters(uint16_t address, uint8_t *buffer, uint16_t size)
{
	...
	llcc68_spi_transceive(req, NULL, sizeof(req), buffer, NULL, size);
}
			  ...
			  uint8_t *buffer, uint16_t size)
{
	...
	LOG_DBG("Issuing opcode 0x%x (data size: %" PRIx16 ")",
		opcode, size);
	llcc68_spi_transceive(tx_req, rx_req, sizeof(rx_req),
			      NULL, buffer, size);
...
static void llcc68_set_tx_enable(int value)
{
#if HAVE_GPIO_TX_ENABLE
...
static void llcc68_set_rx_enable(int value)
{
#if HAVE_GPIO_RX_ENABLE
...
{
	LOG_DBG("SetOperatingMode: %s (%i)", llcc68_mode_name(mode), mode);
	...
	switch (mode) {
	case MODE_TX:
		llcc68_set_rx_enable(0);
		llcc68_set_tx_enable(1);
		break;

	case MODE_RX:
	case MODE_RX_DC:
	case MODE_CAD:
		llcc68_set_tx_enable(0);
		llcc68_set_rx_enable(1);
		break;

	case MODE_SLEEP:
		/* Additionally disable the DIO1 interrupt to save power */
		llcc68_dio1_irq_disable(&dev_data);
		__fallthrough;
	default:
		llcc68_set_rx_enable(0);
		llcc68_set_tx_enable(0);
...
void SX126xSetRfTxPower(int8_t power)
{
	...
	llcc68_set_tx_params(power, RADIO_RAMP_40_US);
}

void SX126xWaitOnBusy(void)
{
	while (llcc68_is_busy(&dev_data)) {
		k_sleep(K_MSEC(1));
	}
}
...
void SX126xWakeup(void)
{
	...
	llcc68_dio1_irq_enable(&dev_data);
...
{
	return llcc68_get_dio1_pin_state(&dev_data);
}

static void llcc68_dio1_irq_work_handler(struct k_work *work)
{
...
static const struct gpio_dt_spec llcc68_gpio_reset = GPIO_DT_SPEC_INST_GET(
        0, reset_gpios);
static const struct gpio_dt_spec llcc68_gpio_busy = GPIO_DT_SPEC_INST_GET(
        0, busy_gpios);
static const struct gpio_dt_spec llcc68_gpio_dio1 = GPIO_DT_SPEC_INST_GET(
        0, dio1_gpios);

static int llcc68_lora_init(const struct device *dev)
{
	const struct llcc68_config *config = dev->config;
	int ret;

	LOG_DBG("Initializing llcc68");

	if (llcc68_configure_pin(antenna_enable, GPIO_OUTPUT_INACTIVE) ||
	    llcc68_configure_pin(rx_enable, GPIO_OUTPUT_INACTIVE) ||
	    llcc68_configure_pin(tx_enable, GPIO_OUTPUT_INACTIVE)) {
		return -EIO;
	}

	k_work_init(&dev_data.dio1_irq_work, llcc68_dio1_irq_work_handler);

	ret = llcc68_variant_init(dev);
	if (ret) {
	...
	ret = llcc68_init(dev);
	if (ret < 0) {
		LOG_ERR("Failed to initialize LLCC68 common");
		return ret;
	}
...
void llcc68_reset(struct llcc68_data *dev)
{
    gpio_pin_set_dt(&llcc68_gpio_reset, 1);
    k_sleep(K_MSEC(20));
    gpio_pin_set_dt(&llcc68_gpio_reset, 0);
    k_sleep(K_MSEC(10));
}

bool llcc68_is_busy(struct llcc68_data *dev)
{
    return gpio_pin_get_dt(&llcc68_gpio_busy);
}

uint32_t llcc68_get_dio1_pin_state(struct llcc68_data *dev)
{
    return gpio_pin_get_dt(&llcc68_gpio_dio1) > 0 ? 1U : 0U;
}

void llcc68_dio1_irq_enable(struct llcc68_data *dev)
{
    gpio_pin_interrupt_configure_dt(&llcc68_gpio_dio1,
                                    GPIO_INT_EDGE_TO_ACTIVE);
}

void llcc68_dio1_irq_disable(struct llcc68_data *dev)
{
    gpio_pin_interrupt_configure_dt(&llcc68_gpio_dio1,
                                    GPIO_INT_DISABLE);
}

static void llcc68_dio1_irq_callback(const struct device *dev,
                                     struct gpio_callback *cb, uint32_t pins)
{
    struct llcc68_data *data = CONTAINER_OF(cb, struct llcc68_data,
                                                dio1_irq_callback);

    if (pins & BIT(llcc68_gpio_dio1.pin)) {
        k_work_submit(&data->dio1_irq_work);
    }
}
...
int llcc68_variant_init(const struct device *dev)
{
    ...
    if (gpio_pin_configure_dt(&llcc68_gpio_reset, GPIO_OUTPUT_ACTIVE) ||
        gpio_pin_configure_dt(&llcc68_gpio_busy, GPIO_INPUT) ||
        gpio_pin_configure_dt(&llcc68_gpio_dio1, GPIO_INPUT)) {
        LOG_ERR("GPIO configuration failed.");
        return -EIO;
    }

    gpio_init_callback(&data->dio1_irq_callback,
                       llcc68_dio1_irq_callback, BIT(llcc68_gpio_dio1.pin));
    if (gpio_add_callback(llcc68_gpio_dio1.port,
                          &data->dio1_irq_callback) < 0) {
...
static const struct lora_driver_api llcc68_lora_api = {
	.config = llcc68_lora_config,
	.send = llcc68_lora_send,
	.send_async = llcc68_lora_send_async,
	.recv = llcc68_lora_recv,
	.recv_async = llcc68_lora_recv_async,
    .test_cw = llcc68_lora_test_cw,
    .is_channel_free = llcc68_lora_is_channel_free,
};

DEVICE_DT_INST_DEFINE(0, &llcc68_lora_init, NULL, &dev_data,
